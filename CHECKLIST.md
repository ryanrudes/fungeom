...
**Current status:** 491 tests · **100 % line coverage** (enforced via
...
| Timeline / TimeMap | ✅ | 2026-06-23 | Added (Timeline) `to_master`→`TimeMap` (partial ungrounded) and `relative_to(other)`→`TimeMap` (partial: either ungrounded, or `other` a frozen zero-rate reference — must invert it) — mirror Frame.relative_to. TimeMap surface already complete; deferred `TimeMap.apply(instant)` (≈ `Timeline.at`). |
...
(affine time-warp by a `TimeMap` — shift / scale / reverse; Unresolvable at zero
rate), `restrict` (narrow the support to an `Interval` *or* `Coverage`'s overlap;
...
logic (`decide_sampled`/`_sample`/`_resampled`/`_reparameterized`/`_restricted`,
