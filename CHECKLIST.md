...
**Current status:** 1273 tests · **100 % line coverage** (enforced via
...
> each primitive's ledger note. **Still pending:** the sweep on the **over-time signal facades**
> (`ScalarSignal`/`Vec3Signal`/…/the bundle signals) — built but not yet swept for missing
> combinators.
