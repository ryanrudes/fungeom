...
**Current status:** 1356 tests · **100 % line coverage** (enforced via
...
| `Point3BundleSignal.fit_convex_face(tolerance=)` → `FaceSignal` (per-frame plane fit + convex hull in its chart — the **bounded** moving patch) | `_FittedFaceSignal` (`signals/face.py`); `FaceSignal` split into a facade over `_TransportedFaceSignal` / `_FittedFaceSignal`, and the five batched readbacks now guard on the transported kind and defer to the generic path otherwise | ✅ | ✅ | ✅ | ✅ (any degenerate frame → whole signal, strict like `fit_plane`; `region()` Unresolvable for a fitted patch — no single static footprint) | ✅ | ✅ |
