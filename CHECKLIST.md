...
**Current status:** 1496 tests · **100 % line coverage** (enforced via
...
| Point3 | ✅ | 2026-06-23 | Added `transformed_by` (rigid motion), `reflect_across` (central symmetry) — both total. Deferred: `barycentric` (≡ `affine`), `as_vector_from` (≡ `displacement_to`), reframe/express-in-frame (value-level `Point3Value.to_frame` exists; resolver form needs a frame-typed target API). **`free(identity)` shipped in 0.4.0** (the late-bound leaf — its own row + the free-variables row in Core machinery; `docs/free-variables.md`). **Re-audit (2026-06-29):** the deferred `reframe/express-in-frame` now *earns its place* — `Frame` is mature and returning a `Vec3` (the coordinate triple), not a `Point3`, sidesteps the world-anchoring invariant — so added **`coordinates_in(frame)→Vec3`**, the read-side inverse of the `in_frame` constructor (partial: ungrounded frame; mirrors the `DisplacementVec3` cross-type pattern), mirrored on `Point2`. Remaining deferrals hold (`barycentric`≡`affine`, `as_vector_from`≡`displacement_to`). |
...
| Point2 | ✅ | 2026-06-24 | **No gaps** — the surface mirrors the already-audited `Point3` exactly (`at`/`in_frame`/`centroid`/`affine`; `translate`/`lerp`/`midpoint`/`displacement_to`/`distance_to`/`direction_to`/`transformed_by`/`reflect_across`). Point3's deferrals apply identically (`barycentric`≡`affine`, `as_vector_from`≡`displacement_to`, resolver-form reframe needs a frame-typed target API). No 2D-specific gap (a signed angle at a vertex composes via `direction_to(...).signed_angle_to(...)`). **Re-audit (2026-06-29):** added **`coordinates_in(frame)→Vec2`** to keep the mirror with Point3's new read-side inverse of `in_frame` (partial: ungrounded frame; concrete `LocalCoordinates2`). The rest still mirrors Point3 and its remaining deferrals hold. |
...
| `coordinates_in` | `LocalCoordinates2` → `Vec2` | ✅ | ✅ | ✅ | ✅ (ungrounded frame) | ✅ | ✅ |
...
| `coordinates_in` | `LocalCoordinates3` → `Vec3` | ✅ | ✅ | ✅ | ✅ (ungrounded frame) | ✅ | ✅ |
...
| `Point3BundleSignal.hull_in(PlaneSignal, tolerance=)` → `FaceSignal` (this cloud's convex hull taken in *that* plane's chart, per frame — the **bounded** moving patch whose surface is decided elsewhere) | `_HulledFaceSignal` (`signals/face.py`) → `decide_lifted` over `Face.on(plane.at(t), TolerantBundleHullRegion2(cloud.at(t).in_frame(plane.at(t))))`; `FaceSignal` is a facade over `_TransportedFaceSignal` / `_HulledFaceSignal`, and the five batched readbacks guard on the transported kind and defer to the generic path otherwise | ✅ | ✅ | ✅ | ✅ (at any aligned instant: < 3 present points, an empty cloud, a chart projection collinear within `tolerance`, an unresolvable plane — strict, one bad instant → whole signal; plus disjoint supports; `region()` Unresolvable for a per-frame patch — no single static footprint) | ✅ (cloud **and** plane) | ✅ |
| `Point3BundleSignal.fit_convex_face(tolerance=)` → `FaceSignal` (per-frame plane fit + convex hull in its chart — the *fused* case) | **defined as** `self.hull_in(self.fit_plane(tolerance=t), tolerance=t)` — no concrete of its own, so the two cannot drift | ✅ | ✅ | ✅ | ✅ (any degenerate frame → whole signal, strict like `fit_plane`) | ✅ | ✅ |
| over-time `where(keys|Roster)` on `Point3BundleSignal` / `TransformBundleSignal` — the **entity-axis** counterpart of `restrict` (which narrows time) | `_Where…BundleSignal` → `decide_where_over_time` (reuses the value-level `narrowed`, so a bundle and a bundle-over-time cannot drift apart) | ✅ | ✅ | ✅ | ✅ (unresolvable source or `Roster`; narrowing to nothing is a valid *empty* collection, not a gap) | ✅ | ✅ |
| over-time `relabel(RosterMap)` on `Point3BundleSignal` / `TransformBundleSignal` — the identity transfer across time, i.e. **what retargeting is** | `_Relabeled…BundleSignal` → `decide_relabeled_over_time` (reuses the value-level `renamed`) | ✅ | ✅ | ✅ | ✅ (collapses two keys onto one target; unresolvable source or map) | ✅ | ✅ |
