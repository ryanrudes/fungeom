...
| collections: `Point3Bundle` (a point cloud) | `of` (points, keyed by position or explicit keys), `from_array` (a raw `(N, 3)` array in a shared frame), `from_map` (a `{key: point}` mapping, optionally over a wider `roster` so the missing keys read as *absent*) | `at` (→ `Point3`), `present` (→ `Bool`), `count` (→ `Scalar`), `where` (sub-cloud), `centroid` (→ `Point3`, folds over present members) | malformed build (key/value count mismatch, duplicate keys, an ungrounded member); `at` an absent or unknown key; `centroid` of no present members |
