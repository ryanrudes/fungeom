...
| `Region2` (bounded planar area — the 2D sibling of `Coverage`) | `rectangle`, `disc` (polygon-approximated), `polygon` (a simple polygon), `hull` (convex hull of points / a `Point2Bundle`), `empty` | `contains` (→ `Bool`, boundary included), `area` (→ `Scalar`), `centroid` (→ `Point2`), `vertices` / `bounds` (→ `Point2Bundle`), `signed_distance` (→ `Scalar`, **positive inside** — the balance-board / ZMP margin) / `nearest_boundary_point` (→ `Point2`), `sample` (N boundary points → `Point2Bundle`) / `corners` (vertices → `Point2Bundle`), `offset` (grow ≥0 / erode <0, general — Minkowski buffer via GEOS; erosion past extinction → empty), `union` / `intersection` / `difference` (**general & total** — arbitrary simple polygons + holes, via GEOS: disjoint union → multipolygon, contained `difference` → a punched hole, partial overlaps clip correctly), `transformed_by` (rigid 2D motion) | non-positive `rectangle`/`disc` extent; `polygon` that is self-intersecting / < 3 vertices / zero-area / has coincident vertices; `hull` of < 3 non-collinear points; `centroid`/`bounds`/`signed_distance`/`nearest_boundary_point` of the empty region; `sample` of a non-positive count / empty region. (The boolean ops & `offset` are **total** — only an `Unresolvable` operand propagates.) |
| `Face` (oriented bounded patch — `Plane` + `Region2`) | `on` (a plane + a region in its chart) | `plane` (→ `Plane`) / `region` (→ `Region2`), `closest_point` (→ `Point3`, **clamped into the region**), `clearance` (→ `Scalar`, the honest 3-D distance to the *bounded* patch — right when the foot is beside, not above) | ungrounded plane / degenerate region propagate; `closest_point`/`clearance` of an empty-region face |
